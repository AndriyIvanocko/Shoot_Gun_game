...
public class NextLevel implements Screen {
    ...
    public NextLevel(MyGame game) {
        ...
        Label gameOver = new Label("NEXT LEVEL",style);
        Label click = new Label("Click to next level", style);
    ...
    public void render(float delta) {
        if(Hud.level == 2) {
            if (Gdx.input.justTouched()) {
                game.setScreen(new Level_Screen_Two(game));
            }
        }
        if( Hud.level == 3){
            if (Gdx.input.justTouched()) {
                game.setScreen(new LevelScreenThree(game));
            }
        }
